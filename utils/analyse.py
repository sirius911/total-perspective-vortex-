...
def analyse(subject:int, n_experience:int, drop_option, options):
    ...
    ica = mne.preprocessing.ICA(n_components=20, random_state=0, method='infomax')
    ...
    ica.apply(raw.copy(), exclude=ica.exclude).plot(scalings=dict(eeg=250e-6),title="After")
    plt.show()
